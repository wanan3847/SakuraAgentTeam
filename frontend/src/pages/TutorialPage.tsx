...
const DOWNLOAD_SECTIONS: DownloadMethod[] = [
  {
    ...
    iconName: 'Globe',
    blocks: [
      { name: '在线访问', cmd: 'https://sakura.041126.xyz' },
  ...
  },
  {
    ...
    req: 'Apple Silicon / Intel 通用',
    iconName: 'Apple',
    blocks: [
      { name: '下载 .dmg', cmd: 'curl -L -O https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/SakuraAgentTeam-mac.dmg\nopen SakuraAgentTeam-mac.dmg' },
      { name: 'Homebrew', cmd: 'brew install --cask sakura-agent-team', note: '在 tap 接受后可用' },
    ],
    verify: 'sakura --version  # 应输出版本号',
  },
  {
    id: 'windows',
    title: 'Windows 桌面端',
    ...
    iconName: 'Monitor',
    blocks: [
      { name: '下载 .exe 安装器', cmd: 'curl -L -O https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/SakuraAgentTeam-setup.exe\nSakuraAgentTeam-setup.exe' },
      { name: 'winget', cmd: 'winget install SakuraAgentTeam' },
      { name: 'Scoop', cmd: 'scoop install sakura-agent-team' },
      { name: 'Chocolatey', cmd: 'choco install sakura-agent-team' },
    ],
    verify: 'sakura --version  # 任意终端执行',
  },
  {
    id: 'linux',
    title: 'Linux 桌面端',
    ...
    iconName: 'Terminal',
    blocks: [
      { name: 'AppImage（通用）', cmd: 'curl -L -O https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/SakuraAgentTeam-x86_64.AppImage\nchmod +x SakuraAgentTeam-x86_64.AppImage\n./SakuraAgentTeam-x86_64.AppImage' },
      { name: 'Debian / Ubuntu', cmd: 'curl -L -O https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/sakura-agent-team.deb\nsudo dpkg -i sakura-agent-team.deb' },
      { name: 'Fedora / RHEL', cmd: 'sudo dnf install https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/sakura-agent-team.rpm' },
      { name: 'Arch / Manjaro (AUR)', cmd: 'yay -S sakura-agent-team' },
      { name: 'Flatpak', cmd: 'flatpak install flathub io.github.wanan3847.SakuraAgentTeam' },
    ],
    verify: 'sakura --version',
  },
  {
    id: 'vscode',
    title: 'VS Code 插件',
    ...
    iconName: 'Code2',
    blocks: [
      { name: '从 Marketplace 安装', cmd: 'ext install sakura-agent-team.sakura-agent-team' },
      { name: '从 .vsix 安装', cmd: 'curl -L -O https://github.com/wanan3847/SakuraAgentTeam/releases/latest/download/sakura-agent-team.vsix\ncode --install-extension sakura-agent-team.vsix' },
      { name: '从源码打包', cmd: 'cd vscode-extension && npm install && npm run package' },
    ],
    verify: 'VS Code 侧边栏出现樱花小队图标，Ctrl+Shift+P → "Sakura: Open Chat"',
  },
  {
    id: 'cli',
    title: 'CLI 命令行',
    ...
    iconName: 'Terminal',
    blocks: [
      { name: 'pip（推荐）', cmd: 'pip install sakura-agent-team' },
      { name: 'pipx（隔离安装）', cmd: 'pipx install sakura-agent-team' },
      { name: 'uv', cmd: 'uv tool install sakura-agent-team' },
      { name: 'Homebrew (macOS/Linux)', cmd: 'brew install sakura-agent-team' },
      { name: 'Scoop (Windows)', cmd: 'scoop install sakura-agent-team' },
    ],
    verify: 'sakura --version  # 应输出 sakura-agent-team x.y.z',
  },
  {
    id: 'oneclick',
    title: '一键安装脚本',
    ...
    verify: '脚本会同时安装 CLI + 创建 sakura 命令别名，提示 "Install complete" 即成功。',
  },
  {
    id: 'docker',
    title: 'Docker',
    ...
    iconName: 'Container',
    blocks: [
      { name: 'docker run 后端', cmd: 'docker run -d --name sakura-backend -p 8000:8000 \\\n  -e OPENAI_API_KEY=sk-... \\\n  ghcr.io/wanan3847/sakura-agent-team-backend:latest' },
    ...
    verify: 'curl http://localhost:8000/docs  # 看到 Swagger 文档即成功',
  },
  {
    id: 'source',
    title: '源码安装',
    platform: '需要 Node 20+ 和 Python 3.11+',
    iconName: 'GitBranch',
    blocks: [
      { name: '克隆 + 启动', cmd: 'git clone https://github.com/wanan3847/SakuraAgentTeam.git\ncd SakuraAgentTeam\n./deploy.sh dev' },
      { name: '手动启动 (后端)', cmd: 'cd backend && pip install -r requirements.txt\npython -m uvicorn app.api.main:app --reload --port 8000' },
      { name: '手动启动 (前端)', cmd: 'cd frontend && npm install && npm run dev' },
    ],
