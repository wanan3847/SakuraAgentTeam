...
三个内置实现，统一接口 `LLMProviderFactory.create(provider, model, api_key, base_url)`：

- `openai` / `anthropic` — 旧版纯 SDK（轻量）
- `litellm` — **推荐** — 100+ 提供商统一接口（OpenAI / Anthropic / DeepSeek / Qwen / Zhipu / Ollama / Bedrock / Gemini / Groq / OpenRouter … 2784+ 模型）

切换：在 `.env` 设 `DEFAULT_LLM_PROVIDER=litellm` + `DEFAULT_LLM_MODEL=openai/gpt-4o`（或 `deepseek/deepseek-chat` / `ollama/llama3` 等）。
API Key 通过对应环境变量（`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `DASHSCOPE_API_KEY` …）或 `api_key` 参数传入。
查询：`python3 -m cli providers`（24 常用）· `--full`（2784 全量）。
